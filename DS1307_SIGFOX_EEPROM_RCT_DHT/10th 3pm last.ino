...
volatile bool recordingData = true;
int numReadings = 0;
...
//
void printLastRecordingData() {
  int address = (numReadings - 60) * (sizeof(float) + sizeof(unsigned long)); // Calculate the starting address of the last 2 minutes of data

  // Ensure the address is not negative (in case there's not enough data recorded yet)
  if (address < 0) {
    address = 0;
  }

  while (address < numReadings * (sizeof(float) + sizeof(unsigned long))) {
    TemperatureReading reading;
    readTemperatureFromEEPROM(address, reading);

    // Print the temperature and timestamp
    Serial.print("Address: ");
    Serial.print(reading.address);
    Serial.print(", Temperature: ");
    Serial.print(reading.temperature);
    ...
    address += dataSize; // Move to the next data block in EEPROM
  }
}
...
void loop() {
  if (Read_store == 1 && !recordingData) {
    ...
    // Write the temperature reading to EEPROM in the main loop
    writeTemperatureToEEPROM(reading.address, reading.temperature, reading.timestamp);

    numReadings++;

    // Check if 30 seconds) have elapsed, and if so, stop recording
    if (numReadings >= 3) { // 10 seconds * 3 = 30 seconds
      recordingData = false;
    }
    ...
    printLastRecordingData(); // Print the last 30 sec of data
    delay(5000);
  }
}
